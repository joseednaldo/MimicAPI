...
namespace MimicAPI.V1.Repositories
{
    public class PalavraRepository : IPalavraRepository
    {
        ...
        public PaginationList<Palavra> ObterPalavras(PalavraUrlQuery query)
        {
            ...
            {
                item = item.Where(a => a.DtCriacao > query.Data.Value || a.DtAtualizacao > query.Data.Value);
            }

            if (query.PagNumero.HasValue)
            {
                var qauntidadeTotalRegsitros = item.Count();
                ...
                paginacao.TotalResgistros = qauntidadeTotalRegsitros;
                paginacao.TotalPaginas = (int)Math.Ceiling((double)qauntidadeTotalRegsitros / query.PagRegistro.Value);
